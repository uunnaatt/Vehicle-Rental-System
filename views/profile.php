    ...
    try {
        ...
        container.innerHTML = data.records.map(b => `
            <div style="background:rgba(255,255,255,0.2); border-radius:16px; padding:20px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:15px; box-shadow:0 4px 15px rgba(0,0,0,0.08);">
                <div>
                    <h3 style="font-size:18px; font-weight:700; margin-bottom:4px;"><i class="fa-solid fa-car"></i> ${b.vehicle_name}</h3>
                    <p style="font-size:14px; opacity:0.75;"><i class="fa-regular fa-calendar"></i> ${b.start_date} &rarr; ${b.end_date}</p>
        ...
        container.innerHTML = data.records.slice(0, 5).map(r => `
            <div style="background:rgba(255,255,255,0.2); border-radius:16px; padding:20px; box-shadow:0 4px 15px rgba(0,0,0,0.08);">
                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:8px;">
                    <h4 style="font-weight:700;"><i class="fa-solid fa-car"></i> ${r.vehicle_name || 'Vehicle'}</h4>
    ...
    try {
        ...
        if (data.records && data.records.length > 0) {
            container.innerHTML = data.records.map(m => {
                // If I sent it (receiver is admin), align right. If admin sent it, align left.
                const isMine = (String(m.receiver_id) === String(adminId));
                return `<div style="max-width:80%; align-self:${isMine?'flex-end':'flex-start'}; background:${isMine?'#38bdf8':'rgba(255,255,255,0.3)'}; color:${isMine?'#0f172a':'inherit'}; padding:10px 15px; border-radius:15px; font-size:14px; word-break:break-word;">${m.message}</div>`;
            }).join('');
...
async function sendChatMessage() {
    ...
    // Optimistic UI: show message immediately in blue bubble
    const container = document.getElementById('chat-messages');
    // Clear placeholder text on first send
    if (container.querySelector('p')) container.innerHTML = '';
    const bubble = document.createElement('div');
    bubble.textContent = msg;
    bubble.style.cssText = 'max-width:80%; align-self:flex-end; background:#38bdf8; color:#0f172a; padding:10px 15px; border-radius:15px; font-size:14px; word-break:break-word;';
    ...
    try {
        ...
        // Reload to sync with server (catches admin replies too)
        loadChatMessages();
    } catch(e) {
